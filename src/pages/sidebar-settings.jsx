    ...
    <aside className="w-80 h-screen bg-[#3D523E] p-6 text-white font-sans overflow-y-auto custom-scrollbar">
        ...
        <div className={`rounded-xl overflow-hidden transition-all ${openAccount ? 'bg-[#F5F2EA] shadow-lg' : ''}`}>
          ...
          {openAccount && (
            <div className="flex flex-col py-3 px-4 space-y-3">
              {[
                { label: "Company Information", path: "/company-information" },
                { label: "Profile Information", path: "/profile-basic" },
                { label: "Region Settings", path: "/regional-settings" },
                { label: "Points of Contact", path: "/points-contact" },
      ...
      <nav className="space-y-2">
        ...
        <MenuItem 
          ...
          navigate={navigate}
          children={[
            { label: "User Roles", path: "/user-teams" },
             { label: "Add User Roles", path: "/add-user" },
        ...
        <MenuItem 
          ...
          children={[
            ...
            { label: "Catalog", path: "/catlog-Library" },
            { label: "Supplier Types", path: "/supplier-types", active: true },
            { label: "Supplier Directory", path: "/supplierDirectories" },
            { label: "Email Subscription", path: "/email-subscription" },
            { label: "Supplier Questionnaire", path: "/supplier-questionnaire" },
            { label: "Questionnaire Responses", path: "/questionnaire-response" }
          ]}
        />
        ...
        <MenuItem 
          ...
          navigate={navigate}
          children={[
            { label: "BID Settings", path: "/bid-settings" },
            { label: "Purchase Order Defaults", path: "/purchase-order-default" },
            { label: "Purchase Order Approval Workflow", path: "/purchase-order-workflow" }
          ]}
        />
      </nav>
    </aside>
